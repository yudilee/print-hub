...
@section('title', 'Edit Print Queue: ' . $profile->name)
...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
    <div>
        <h2 class="text-base sm:text-lg font-bold text-white flex items-center gap-2">
            <span>✏️ Edit Print Queue: {{ $profile->name }}</span>
        </h2>
        <p class="text-xs text-slate-400">Modify paper size, margins, duplex mode, and hardware destination</p>
    </div>
...
<form action="{{ route('admin.profiles.update', $profile) }}" method="POST">
        ...
        <div class="space-y-5">
            ...
            <div class="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-4 shadow-xs">
                ...
                <div>
                    ...
                    <span class="text-[10px] text-slate-500 mt-1 block">Lower case, underscores only, no spaces. Must match queue in Odoo.</span>
                </div>
            ...
            <div class="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-4 shadow-xs">
                ...
                <div>
                    ...
                    <select id="routing_type" onchange="toggleRoutingType(this.value)" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200 focus:outline-none focus:border-blue-500">
                        <option value="single" {{ !old('pool_id', $profile->pool_id) ? 'selected' : '' }}>Single Assigned Workstation Printer</option>
                        <option value="pool" {{ old('pool_id', $profile->pool_id) ? 'selected' : '' }}>Pooled Printers (Failover / Load Balanced)</option>
                    </select>
                </div>
                ...
                <div id="agent_group" class="space-y-4">
                    ...
                    <div>
                        ...
                        <div id="printer_input_container">
                            <input type="text" name="default_printer" id="default_printer" 
                                   value="{{ old('default_printer', $profile->default_printer) }}"
                                   class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200 focus:outline-none focus:border-blue-500">
                        </div>
                    </div>
                </div>
            ...
            <div class="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-4 shadow-xs">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="paper_size" id="paper_size" required onchange="toggleCustomPaper(this.value)"
                            ...
                            @foreach(['A4 (210×297mm)', 'A5 (148×210mm)', 'Letter (8.5×11in)', 'Legal (8.5×14in)', 'Continuous Form 9.5x11in', 'Thermal 80mm Receipt', 'Thermal 58mm Receipt', 'CUSTOM'] as $size)
                                <option value="{{ $size }}" {{ old('paper_size', $profile->paper_size) == $size ? 'selected' : '' }}>
                                    {{ $size }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="orientation" id="orientation" required class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200 focus:outline-none focus:border-blue-500">
                            <option value="portrait" {{ old('orientation', $profile->orientation) == 'portrait' ? 'selected' : '' }}>Portrait (Tegak)</option>
                            <option value="landscape" {{ old('orientation', $profile->orientation) == 'landscape' ? 'selected' : '' }}>Landscape (Memanjang)</option>
                        </select>
                    </div>
                </div>
                ...
                <div id="custom_paper_container" class="hidden p-3.5 rounded-xl bg-slate-950 border border-slate-800 space-y-3">
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="number" step="0.1" name="custom_width" value="{{ old('custom_width', $profile->custom_width) }}" class="w-full px-3 py-1.5 bg-slate-900 border border-slate-800 rounded-xl text-xs text-slate-200">
                        </div>
                        <div>
                            <label class="block text-[11px] text-slate-400 mb-1">Height (mm)</label>
                            <input type="number" step="0.1" name="custom_height" value="{{ old('custom_height', $profile->custom_height) }}" class="w-full px-3 py-1.5 bg-slate-900 border border-slate-800 rounded-xl text-xs text-slate-200">
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-4 gap-2">
                        <div>
                            ...
                            <input type="number" step="0.5" name="margin_top" value="{{ old('margin_top', $profile->margin_top ?? 0) }}" class="w-full px-2 py-1.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200 text-center">
                        </div>
                        <div>
                            <span class="text-[10px] text-slate-500 block mb-0.5">Bottom</span>
                            <input type="number" step="0.5" name="margin_bottom" value="{{ old('margin_bottom', $profile->margin_bottom ?? 0) }}" class="w-full px-2 py-1.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200 text-center">
                        </div>
                        <div>
                            <span class="text-[10px] text-slate-500 block mb-0.5">Left</span>
                            <input type="number" step="0.5" name="margin_left" value="{{ old('margin_left', $profile->margin_left ?? 0) }}" class="w-full px-2 py-1.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200 text-center">
                        </div>
                        <div>
                            <span class="text-[10px] text-slate-500 block mb-0.5">Right</span>
                            <input type="number" step="0.5" name="margin_right" value="{{ old('margin_right', $profile->margin_right ?? 0) }}" class="w-full px-2 py-1.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200 text-center">
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="tray_source" id="tray_source" placeholder="e.g. Tray 1, Manual Feed, Cassette (optional)"
                           value="{{ old('tray_source', $profile->tray_source) }}"
                           class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200 focus:outline-none focus:border-blue-500">
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="duplex" id="duplex" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200 focus:outline-none focus:border-blue-500">
                            <option value="one-sided" {{ old('duplex', $profile->duplex) == 'one-sided' ? 'selected' : '' }}>One-Sided (Simplex / Satu Sisi)</option>
                            <option value="two-sided-long" {{ old('duplex', $profile->duplex) == 'two-sided-long' ? 'selected' : '' }}>Two-Sided (Duplex Long Edge / Bolak-Balik Panjang)</option>
                            <option value="two-sided-short" {{ old('duplex', $profile->duplex) == 'two-sided-short' ? 'selected' : '' }}>Two-Sided (Duplex Short Edge / Bolak-Balik Pendek)</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" min="1" max="99" name="copies" id="copies" required
                               value="{{ old('copies', $profile->copies ?? 1) }}"
                               class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200 focus:outline-none focus:border-blue-500">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="color_mode" id="color_mode" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200 focus:outline-none focus:border-blue-500">
                            <option value="monochrome" {{ old('color_mode', $profile->color_mode) == 'monochrome' ? 'selected' : '' }}>Monochrome (Hitam Putih)</option>
                            <option value="color" {{ old('color_mode', $profile->color_mode) == 'color' ? 'selected' : '' }}>Color (Warna)</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="watermark_text" id="watermark_text" placeholder="e.g. COPY / CONFIDENTIAL (optional)"
                               value="{{ old('watermark_text', $profile->watermark_text) }}"
                               class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200 focus:outline-none focus:border-blue-500">
                    </div>
                </div>
            </div>
    ...
    <div class="p-4 rounded-2xl bg-slate-900 border border-slate-800 flex items-center justify-between shadow-lg">
        ...
        <button type="submit" class="btn-primary btn-sm flex items-center gap-1.5 px-6 shadow-lg shadow-blue-500/25">
            ...
            <span class="font-bold">Update Print Queue</span>
        </button>
    </div>
</form>
...
function updatePrinterDropdown(agentId) {
    ...
    const currentVal = "{{ old('default_printer', $profile->default_printer) }}";

    if (!agentId) {
        container.innerHTML = `<input type="text" name="default_printer" id="default_printer" value="${currentVal}" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200">`;
        return;
    }
